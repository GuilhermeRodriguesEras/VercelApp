...
def gerar_pdf_proposta(dados_front, dados_orcamento, contato, orcamento_id):
    """Gera o PDF local decompondo descrições longas em sub-linhas

    para permitir a quebra de página fluida exatamente como no Tiny.
    """
    ...
    def html_tiny_para_reportlab(texto_html):
        if not texto_html:
            return ""
        texto_clean = unescape(str(texto_html))
        # Remove tags HTML não suportadas pelo Paragraph do ReportLab
        texto_clean = re.sub(r"<(?!b|/b|i|/i|u|/u|font|/font)[^>]+>", "", texto_clean)
        return texto_clean

    def quebrar_descricao_em_linhas(texto_raw):
        """Quebra textos ou HTMLs longos em uma lista de linhas/parágrafos

        curtos para que cada um virar uma linha individual na tabela do ReportLab.
        """
        if not texto_raw:
            return []

        texto_str = str(texto_raw)
        # Converter quebras de bloco HTML em \n
        texto_str = re.sub(r"<\s*br\s*/?\s*>", "\n", texto_str, flags=re.IGNORECASE)
        texto_str = re.sub(r"<\s*/\s*(p|div|li)\s*>", "\n", texto_str, flags=re.IGNORECASE)

        linhas_raw = [l.strip() for l in texto_str.split("\n") if l.strip()]
        linhas_finais = []

        for linha in linhas_raw:
            linha_limpa = html_tiny_para_reportlab(linha)
            if not linha_limpa:
                continue

            # Se um bloco for excessivamente longo sem quebras, força divisão a cada ~200 caracteres
            texto_sem_tags = re.sub(r"<[^>]+>", "", linha_limpa)
            if len(texto_sem_tags) > 220:
                palavras = linha_limpa.split(" ")
                chunk = []
                chunk_len = 0
                for p in palavras:
                    chunk.append(p)
                    chunk_len += len(re.sub(r"<[^>]+>", "", p)) + 1
                    if chunk_len >= 180:
                        linhas_finais.append(" ".join(chunk))
                        chunk = []
                        chunk_len = 0
                if chunk:
                    linhas_finais.append(" ".join(chunk))
    ...
    logo = Image(logo_stream, width=36 * mm, height=26 * mm)
    ...
    story.append(Paragraph("Itens de produto ou serviço", section_title_style))
    ...
    # Construção da tabela decompondo os textos em linhas para permitir quebra perfeita
    for indice, item in enumerate(itens, start=1):
        ...
        # Extrai todas as sub-linhas da descrição complementar
        linhas_desc = []
        if complemento and unescape(complemento).strip() != unescape(descricao).strip():
            linhas_desc = quebrar_descricao_em_linhas(complemento)
        else:
            linhas_desc = quebrar_descricao_em_linhas(descricao)
            if linhas_desc:
                descricao = linhas_desc[0]
                linhas_desc = linhas_desc[1:]

        # Índice da primeira linha deste produto
        row_idx = len(corpo_itens)
        item_start_indices.append(row_idx)

        # 1ª Linha do item (Título + Valores nas colunas)
        corpo_itens.append([
            Paragraph(str(indice), table_center),
            Paragraph(f"<b>{escape_html(descricao)}</b>", table_left),
        ...
        # Sub-linhas da descrição (apenas preenchem a coluna "Item", mantendo as outras vazias)
        for linha_txt in linhas_desc:
            corpo_itens.append([
                "",  # Nº vazio
                Paragraph(linha_txt, table_left),
    ...
    tabela_itens = Table(
        ...
        repeatRows=0,  # 0 para corresponder à imagem onde a tabela continua diretamente no topo da folha 2
        splitByRow=1,
    )
    ...
    t_style = [
        ...
        # Linhas verticais contínuas para cada uma das colunas
        ("LINEBEFORE", (0, 0), (0, -1), 0.6, colors.black),
        ("LINEBEFORE", (1, 0), (1, -1), 0.6, colors.black),
        ("LINEBEFORE", (2, 0), (2, -1), 0.6, colors.black),
        ("LINEBEFORE", (3, 0), (3, -1), 0.6, colors.black),
        ("LINEBEFORE", (4, 0), (4, -1), 0.6, colors.black),
        ("LINEBEFORE", (5, 0), (5, -1), 0.6, colors.black),
        ("LINEBEFORE", (6, 0), (6, -1), 0.6, colors.black),
        ("LINEAFTER", (6, 0), (6, -1), 0.6, colors.black),

        # Bordas horizontais do cabeçalho
        ("LINEABOVE", (0, 0), (-1, 0), 0.6, colors.black),
        ("LINEBELOW", (0, 0), (-1, 0), 0.6, colors.black),
    ]

    # Linha horizontal acima de cada novo produto
    for idx in item_start_indices:
        t_style.append(("LINEABOVE", (0, idx), (-1, idx), 0.6, colors.black))

    # Linhas e mesclagens da linha de Total no fim da tabela
    t_style.append(("LINEABOVE", (0, last_row_idx), (-1, last_row_idx), 0.6, colors.black))
    t_style.append(("LINEBELOW", (0, last_row_idx), (-1, last_row_idx), 0.6, colors.black))
    t_style.append(("SPAN", (0, last_row_idx), (4, last_row_idx)))
    t_style.append(("ALIGN", (5, last_row_idx), (6, last_row_idx), "RIGHT"))

    tabela_itens.setStyle(TableStyle(t_style))
    ...
    story.append(Paragraph("<b>Observações</b>", section_title_style))
